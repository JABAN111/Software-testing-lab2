...
public class TrigonometryFunc implements FuncInterface{
    ...
    // (((((sec(x) + tan(x)) ^ 2) * sin(x)) * ((cot(x) / csc(x)) + sec(x))) * (csc(x) + cot(x)))
    @Override
    public double apply(double x, double accuracy) {
        return (pow(sec.apply(x, accuracy) + tan.apply(x, accuracy), 2) * sin.apply(x, accuracy)) * (cot.apply(x, accuracy) / csc.apply(x, accuracy) + sec.apply(x, accuracy)) * (csc.apply(x, accuracy) + cot.apply(x, accuracy));
    }
